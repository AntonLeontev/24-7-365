<template>
  <div class="card">
    <div class="card-header">
      <div class="row">
        <span class="col-12 col-xxl-4">Список договоров</span>
    ...
    <div class="card-body">
      <div class="table_dark">
        ...
        <div class="table_dark__body">
          <template v-for="contract in contracts.data">
            <div class="table_dark__row" :href="route('users.contract_show', contract.id)">
              <div class="col">
              ...
              <div class="col">{{ contract.amount?.formatted ?? 0 }}</div>
              <div class="col">{{ contract.tariff_title }} - {{ contract.tariff_duration }} мес</div>
              <div class="col">
                <span class="" v-if="contract.status === 'init'">Создан</span>
                <span class="text-success" v-if="contract.status === 'active'">Активен</span>
                <span class="text-danger" v-if="contract.status === 'canceled'">На расторжении</span>
                <span class="" v-if="contract.status === 'pending'">Ждет доплаты</span>
                <span class="" v-if="contract.status === 'finished'">Завершен</span>
                <span class="" v-if="contract.status === 'terminated'">Расторгнут</span>
              </div>
			  <div class="col">{{ this.formatDate(contract.paid_at) }}</div>
            </div>
          </template>
        </div>
      </div>

      <simple-pagination :paginator="contracts" @change-page="changePage"></simple-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    url() {
      let url = new URL(route("contracts.index"));
