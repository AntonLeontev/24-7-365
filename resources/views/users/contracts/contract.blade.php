...
@section('content')
                    ...
                    <div class="card-body d-xl-flex flex-xl-row-reverse justify-content-between gap-4">
                        <div class="d-flex flex-column flex-md-row justify-content-between gap-md-4 w-xl-66 mb-4 gap-0">
                            ...
                            <div class="w-100">
                                ...
                                <x-contract.property value="{{ $contract->paid_at?->translatedFormat('d M Y') }}"
                                    label="Начало" />
                                <x-contract.property
                    ...
                    <div class="p-4 pb-5">
                        <div class="bg-body text-light p-5 text-center">
                            ...
                            @if ($contract->status === $contract::PENDING)
                                <p>
                                    Для активации нужно оплатить <a href="{{ route('invoice.pdf', $contract->payments->first()->id) }}" class="btn-link">счет</a>
                                </p>
                            @endif
                        </div>
                    </div>
    ...
    <x-common.modal id="cancelContract" modalTitle="Закрыть договор №{{ $contract->id }}">
		<div class="mb-5">
			...
			@else
				<p class="text-light">
					Срок действия договора истекает {{ $contract->paid_at?->addMonths($contract->tariff->duration)->translatedFormat('d.m.Y') }}
				</p>
            ...
            <div class="modal-content">
                <div class="modal-header">
					...
					<div class="d-flex gap-3 ">
						<a href="{{ route('users.contract.pdf') }}" class="btn btn-outline-primary d-none d-lg-block">
                ...
                <div class="modal-body">
					<div class="text-bg-dark px-3 py-3 vh-70 overflow-auto">
						@php
							$tariffs = App\Models\Tariff::where('status', App\Models\Tariff::ACTIVE)->get()->groupBy('title');
						@endphp
						@include('pdf.contract', compact('tariffs'))

					</div>
					<a href="{{ route('users.contract.pdf') }}" class="btn btn-outline-primary w-100 d-lg-none mt-4">
						Скачать договор в PDF
					</a>
                </div>
            </div>
